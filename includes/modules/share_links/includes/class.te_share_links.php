...
class C_te_share_links extends C_tableedit_base
{
		...
		if( $mode == 'table' )
		{
			switch( $fieldname )
			{
				...
				case 'url':
				{
					...
					echo '<p><input type="text" id="visual_link_'.$key.'" value="' . nl2br(h(util_tracking_url($value, util_cleanup_keystr('share_link_'.$this->db["name"]), $AI->user->userID, null, util_pretty('share_link_'.$this->db["name"])))) . '" readonly="readonly" class="span12"  /></p>';

					echo '<a href="javascript:void(0);" class="share_links_advanced_link" data-key="'.$key.'"/>Advanced options</a>';
					echo '<div id="share_links_advanced_'.$key.'" class="share_links_advanced" style="display:none;" >';
					...
					//echo '<br/><p><label>SubID</label><input type="text" class="source"  style="width:125px;" value="" placeholder="Enter Your SubID" data-key="'.$key.'" />'.util_qtip($tipval).'</p>';
					echo '<br/><p><label>SubID</label><input type="text" class="source"  style="width:125px;" value="" placeholder="Enter Your SubID" data-key="'.$key.'" /><img width="16" data-toggle="tooltip" data-placement="right" data-html="true" src="images/icons/question_32.png" title="This SubID will be appended to the link below.<br>For reporting purposes, all traffic from this source will be<br>tracked independently from the rest of this campaign."></p>';
					echo '</div>';
					...
					echo '<table  style="margin: 10px 0" cellpadding="0" cellspacing="0">';
					echo '<tr>';
					echo '<td align="left">';
					...
					echo '<tr>';
					echo '<td align="left">';
					echo '<button class="info clip_button1" id="clip_button_'.$key.'" style="display:none; height:30px; padding-top: 5px;" href_txt="'.util_tracking_url($value, util_cleanup_keystr('share_link_'.$this->db["name"]), $AI->user->userID, null, util_pretty('share_link_'.$this->db["name"])).'" tekey="'.$key.'" data-clipboard-target="#visual_link_'.$key.'">Copy Link</button>';
					echo '</td>';
					echo '</tr>';
					echo '</table>';
	...
	function draw_sharing($url,$key)
	{
		...
		$base_domain=substr($AI->get_setting('HTTP_URL'),11,-1);
		//if ( strpos($url, 'http://') === false)
		if ( !preg_match('#^http(s)?://#i', $url) )
		{
			if(strpos($url,$base_domain) === false)
				$url = $AI->get_setting('HTTP_URL') . $url;
			else
				$url='http://'.$url;

		}
		//Facebook link below has to be forced to str to lower or facebook api will view an upper case replicated site as a bad request
		?>
			...
			<span class='st_facebook_large' st_url="<?= strtolower(util_tracking_url($url, util_cleanup_keystr('share_link_'.$this->db["name"]), $AI->user->userID, 'facebook', util_pretty('share_link_'.$this->db["name"]))) ?>"></span>
