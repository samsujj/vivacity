<?php
	...
	class C_te_share_asset extends C_tableedit_base
	{
		...
		function validate_write()
		{
			...
			if( isset( $_FILES['file_upload'] ) && $f->receive('file_upload') )
			{
				...
				if(in_array(strtolower($ext),$filetypes)) {
					...
					if($imagewidth == $_POST['width'] && $imageheight == $_POST['height']){
						...
						$this->write_error_msg = "Please upload exact sized image";
					}


				}else{
					$this->write_error_msg = "File type not allowed";
				}



			}else if(empty($_POST['url'])){
				$this->write_error_msg = "Please uplod banner image";
			}
		...
		function finalize_write()
		{
			...
			util_redirect('share_asset?te_share_link_id='.$this->db['share_link_id']);
		...
		function draw_input_field( $fieldname, $value, $mode, $element_id = '' )
		{
			...
			switch( $fieldname )
			{
				...
				case 'height' : {
					echo '<input type="hidden" name="'.$fieldname.'" id="'.$fieldname.'" value="'.$value.'">';

					echo '<select name="bsize" id="bsize">';
					echo '<option value="" height="" width="">Select banner size</option>';
					echo '<option value="1" height="90" width="728">Leaderboard (728X90)</option>';
					echo '<option value="2" height="60" width="468">Standard Banner (468X60)</option>';
					echo '<option value="3" height="600" width="120">Skyscraper (120X600)</option>';
					echo '<option value="4" height="600" width="160">Wide Skyscraper (160X600)</option>';
					echo '<option value="5" height="60" width="234">Half Banner (234X60)</option>';
					echo '<option value="6" height="90" width="120">Button (120X90)</option>';
					echo '<option value="7" height="60" width="120">Button (120X60)</option>';
					echo '<option value="8" height="125" width="125">Square (125X125)</option>';
					echo '<option value="9" height="31" width="88">Button (88X31)</option>';
					echo '<option value="10" height="240" width="120">Vertical Banner (120X240)</option>';
					echo '</select>';
		...
		function draw_image_list($id=0){
			...
			$pixel_arr = $AI->db->getAll("SELECT * FROM `share_asset` WHERE share_link_id = " . (int) db_in($id) . " AND status=1");
			...
			if(count($pixel_arr)){
				...
				foreach($pixel_arr as $p) {
					...
					$img_url = (file_exists($p['url']))?'<img src="'.$_SERVER['HTTP_HOST'].'/'.$p['url'].'" style="border: solid 1px #ccc; display:block; margin:0 auto; max-width:100%" />':'';

					echo '<div style="width: 96%; padding:20px  2%; border-bottom: solid 1px #ccc;">';
					echo '<h2 style="text-align: center; text-transform: uppercase; text-decoration: underline; margin-bottom: 20px;">'.$p['name'].' ('.$p['width'].'X'.$p['height'].')</h2>';
					echo $img_url;
