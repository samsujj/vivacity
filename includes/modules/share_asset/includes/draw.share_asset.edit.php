...
if(!isset($query['te_share_link_id'])){
	$postURL = $postURL.'&te_share_link_id='.@$_GET['te_share_link_id'];
}
		...
		<fieldset class="te">
			...
			<dl class="te">
				...
				<dt class="te <?php echo $this->get_field_type( 'height' ); ?> height" >
					<label class="te <?php echo $this->get_field_type( 'height' ); ?> height" for="height">Banner Size</label>
				</dt>
			...
			<?php
				...
				{
					?><input class="te te_buttons2 cancle_button" type="button" name="btnCancel" value="Cancel" onclick="document.location='<?php echo htmlspecialchars($this->url( 'te_mode=' . $this->_default_mode_after_save . '&te_key=' . $this->te_key.'&te_share_link_id='. $this->db['share_link_id'] )); ?>';" /><?php
				}
				elseif( $this->te_permit[ $this->_te_modeDefault ] )

				{
					?><input class="te te_buttons2 cancle_button" type="button" name="btnCancel" value="Cancel" onclick="document.location='<?php echo htmlspecialchars($this->url( 'te_mode=' . $this->_te_modeDefault.'&te_share_link_id='. @$_GET['te_share_link_id'] )); ?>';" /><?php
				}
			?>
			</div>
