		...
		?>
			<div class="te te_delete_form">
				<form class="te te_delete_form" name="frm" method="post" action="<?php echo htmlspecialchars($this->url( "te_mode=delete&te_key=" . $this->te_key .'&te_share_link_id='.@$_GET['te_share_link_id'] )); ?>">
					<input class="confirm_buttondel" type="submit" name="btnConfirm" value="Confirm Delete" />
					<input class="confirm_buttondel" type="button" name="btnCancel" value="Cancel" onclick="document.location='<?php echo htmlspecialchars($this->url( 'te_share_link_id=' . @$_GET['te_share_link_id'] )); ?>';" />
				</form>
			</div>
		<?php
	}
