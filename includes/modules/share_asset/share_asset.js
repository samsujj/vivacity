...
$(function(){
	...
	$('#bsize').change(function(){
		var banheight = $(this).find('option:selected').attr('height');
		var banwidth = $(this).find('option:selected').attr('width');

		$('#height').val(banheight);
		$('#width').val(banwidth);

	});

	var banh1 = $('#height').val();
	var banw1 = $('#width').val();

	if(banh1 != '' && banw1 != ''){
		$("#bsize option[height='" + banh1 + "'][width='" + banw1 + "']").attr("selected","selected");
	}
